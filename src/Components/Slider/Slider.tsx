...
//                     {!isMobile && <span className={styles.zoomIcon}><TbZoomInArea /></span>}
//                   </div>
//                 </div>
...
const Slider: React.FC<SliderProps> = ({ images }) => {
  ...
  // Загрузка слайдов из WP если images не переданы
  useEffect(() => {
    if (images) return;
    let mounted = true;
    (async () => {
  ...
  const openModal = (index: number) => {
    ...
    setIsModalOpen(true);
    setIsPlaying(false);
    if (mainSwiperRef.current?.swiper?.autoplay?.running) mainSwiperRef.current.swiper.autoplay.stop();
  };

  const closeModal = () => {
    setIsModalOpen(false);
    setIsPlaying(true);
    if (mainSwiperRef.current?.swiper?.autoplay?.start) mainSwiperRef.current.swiper.autoplay.start();
  };
  ...
  useEffect(() => {
    const onKey = (e: KeyboardEvent) => {
      ...
      else if (e.key === "ArrowLeft" && modalSwiperRef.current) modalSwiperRef.current.swiper.slidePrev();
      else if (e.key === "ArrowRight" && modalSwiperRef.current) modalSwiperRef.current.swiper.slideNext();
    };
  ...
  if (loading) {
    return (
      <div id="works" className={styles.sliderContainer}>
        <h2 className={styles.sliderTitle}>Наши работы</h2>
        <p className={styles.sliderSubtitle}>Качественные тенты для любого транспорта</p>
  ...
  if (!slides.length) return <div className={styles.emptyContainer}>Нет слайдов для отображения</div>;
    ...
    <div id="works" className={styles.sliderContainer}>
      ...
      <div className={styles.mainSliderWrapper}>
        <Swiper
          ...
          freeMode={{ enabled: true, momentum: true, momentumRatio: 1, momentumBounce: false }}
          grabCursor={!isMobile}
          breakpoints={{
              ...
              <div className={styles.slideCard}>
                ...
                >
                  <img
                    ...
                    onError={(e) => {
                      e.currentTarget.src =
                        `data:image/svg+xml;utf8,<svg xmlns="http://www.w3.org/2000/svg" width="400" height="300" viewBox="0 0 400 300"><rect width="400" height="300" fill="%230d3483"/><text x="200" y="150" font-family="Arial" font-size="20" fill="white" text-anchor="middle" dominant-baseline="middle">Тент</text></svg>`;
                      e.currentTarget.alt = "Изображение временно недоступно";
                    }}
                  />
                  <div className={styles.imageOverlay}>
                ...
                <div className={styles.slideInfo}>
                  <p className={styles.slideText}>{image.caption || image.alt}</p>
                </div>
              </div>
            </SwiperSlide>
          ))}
        ...
        <div className={styles.sliderControls}>
          <button className={styles.slider_button_left} aria-label="Предыдущий слайд"><FaArrowLeft/></button>
          <button className={styles.autoplayButton} onClick={toggleAutoplay} aria-label={isPlaying ? "Пауза" : "Воспроизвести"}>
            {isPlaying ? <FaPause /> : <FaPlay />}
          </button>
          <button className={styles.slider_button_right} aria-label="Следующий слайд"><FaArrowRight/></button>
        </div>
      </div>
      ...
      {!isMobile && (
        <Modal
          ...
          onRequestClose={closeModal}
          style={{
            overlay: { backgroundColor: "rgba(0, 0, 0, 0.9)", zIndex: 9999, display: "flex", alignItems: "center", justifyContent: "center" },
            content: { position: "relative", border: "none", background: "transparent", padding: 0, width: "90%", maxHeight: "95vh", borderRadius: "12px", overflow: "hidden" },
          }}
          contentLabel="Просмотр изображения"
        >
          <div className={styles.modalContent}>
            <button className={styles.closeButton} onClick={closeModal} aria-label="Закрыть">✕</button>
            ...
            <Swiper
              ...
              slidesPerView={1}
              initialSlide={selectedImageIndex}
              navigation={{ nextEl: `.${styles.modalNext}`, prevEl: `.${styles.modalPrev}` }}
              className={styles.modalSwiper}
            >
              {slides.map((image) => (
                <SwiperSlide key={`modal-${image.id}`} className={styles.modalSlide}>
                  <div className={styles.modalImageWrapper}>
                    <img src={image.url} alt={image.alt ?? image.caption} className={styles.modalImage} />
                  </div>
                  <div className={styles.modalCaption}>
                    <p>{image.caption || image.alt}</p>
                    <span className={styles.imageCounter}>
                      {slides.findIndex((img) => img.id === image.id) + 1} / {slides.length}
                    </span>
                  </div>
                </SwiperSlide>
              ))}
            ...
            <button className={styles.modalPrev} aria-label="Предыдущее изображение">←</button>
            <button className={styles.modalNext} aria-label="Следующее изображение">→</button>
          </div>
        </Modal>
      )}
    </div>
